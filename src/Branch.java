...
*/
			...
			if(readFile.exists()){
				System.out.println(" Reboot about to becalled !! ");
				rebootCaller();
			}
...
//			getHints();//how to get branchList
			...
			while (true) {
				...
				//timer to check what servers are up to send hints
...
//				System.out.println(bm);
				...
				if(bm.hasDecision()){
					...
					if(decision.equals("commit")){
						...
						//should not be done here. -> checkthis
					...
					if(decision.equals("abort")){
						System.out.println("got into abort message");
					...
					if(decision.equals("readrepair")){
						...
						System.out.println("lates value:"+latestValue);
...
//						File readHintFile = new File(branchFileName);
...
//					new clientThread(readKey);
					new Thread(new Runnable() {
					     public void run() {
					          // code goes here.
//							 System.out.println("got this ReadFlag : "+ readFlag);
...
//													consistentMap.put(readKey, consistentString);
												...
												{
													System.out.println("==> Branch Read not running : "+branch.getIp() + ":"+branch.getPort());
